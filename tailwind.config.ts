...
const generateDarkenColorFrom = (input: string, percent = 0.07): string =>
  colord(input).darken(percent).toHex();

const generateForegroundColorFrom = (input: string, percent = 0.8): string =>
  colord(input)
    .mix(colord(input).isDark() ? 'white' : 'black', percent)
    .toHex();
